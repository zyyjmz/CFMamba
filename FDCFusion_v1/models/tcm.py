...
from compressai.layers import (
    AttentionBlock,
    ResidualBlock,
    ResidualBlockUpsample,          #URB块
    ResidualBlockWithStride,        #DRB块
    conv3x3,
    subpel_conv3x3,
)
...
def get_scale_table(min=SCALES_MIN, max=SCALES_MAX, levels=SCALES_LEVELS):      #该函数可以用于生成一个对数均匀分布的尺度表，这在某些应用场景中非常有用，例如图像处理、音频处理等，其中需要对数据进行对数尺度变换。
    return torch.exp(torch.linspace(math.log(min), math.log(max), levels))

def ste_round(x: Tensor) -> Tensor:             #通过使用直通估计，可以在训练过程中绕过非可微操作，从而使得模型能够正常学习。
    return torch.round(x) - x.detach() + x

def find_named_module(module, query):           #这个函数可以用于在复杂的神经网络模型中快速定位某个特定的模块。这在调试、修改或分析模型时非常有用。例如，你可能想查看某个特定层的权重，或者替换某个层。
...
#管理神经网络模块的缓冲区（buffers）的。缓冲区是模块中的一些持久状态，它们不会被视为模型参数，也不会在训练过程中更新，但它们需要与模型的状态字典（state_dict）保持同步。
def _update_registered_buffer(                      #该函数主要用于在模型加载或更新过程中，根据需要调整或注册模块中的缓冲区。这在迁移学习或模型微调时特别有用，因为可能需要根据新的数据调整缓冲区的大小或内容。
    module,
    buffer_name,
    state_dict_key,
    state_dict,
...
def update_registered_buffers(                      #这个函数是 _update_registered_buffer 的一个高级封装。它接收一个模块和一组缓冲区名称，然后根据状态字典（state_dict）中的数据来更新这些缓冲区。
    module,
    module_name,
    buffer_names,
    state_dict,
...
def Multiplexer(anchor, non_anchor):            #特征重组
    """
...
class WMSA(nn.Module):                  #W-MSA注意力模块
...
class DenseBlock(nn.Module):        #cnn块
    ...
    def __init__(self, in_ch: int, out_ch: int):
        super().__init__()
        self.conv1 = conv3x3(in_ch, out_ch)             #因为用到了密集连接，所以输入通道和输出通道不对等
...
class ConvTransBlock(nn.Module):        #transformer+cnn结构
...
# class SWAtten(AttentionBlock):          #SW-MSA注意力模块
...
class TCM(CompressionModel):    #继承了压缩模型  compressai里面的压缩模型
    def __init__(self, config=[2, 2, 2, 2, 2, 2], head_dim=[8, 16, 32, 32, 16, 8], drop_path_rate=0, N=128,  M=320, num_slices=5, max_support_slices=5, **kwargs):
        ...
        dim = N
        self.M = M
        # 模型可能会多次通过相同的路径，从而导致过拟合。通过在每次通过路径时随机选择一个drop path rate值，可以减少过拟合的风险。
        dpr = [x.item() for x in torch.linspace(0, drop_path_rate, sum(config))]
        begin = 0

        self.m_down1 = [ConvTransBlock(dim, dim, self.head_dim[0], self.window_size, dpr[i+begin], 'W' if not i%2 else 'SW') 
                      for i in range(config[0])] + \
                      [Downblock_SAL(2*N, 2*N, kernel_size=5, stride=2, groups=1, pool_stride=16, num_heads=6)]
        self.m_down2 = [ConvTransBlock(dim, dim, self.head_dim[1], self.window_size, dpr[i+begin], 'W' if not i%2 else 'SW')
                      for i in range(config[1])] + \
                      [Downblock_SAL(2*N, 2*N, kernel_size=5, stride=2, groups=1, pool_stride=8, num_heads=12)] + \
                      [SFMA(in_dim=2 * N, middle_dim=64, adapt_factor=1)]
        self.m_down3 = [ConvTransBlock(dim, dim, self.head_dim[2], self.window_size, dpr[i+begin], 'W' if not i%2 else 'SW')
                      for i in range(config[2])] + \
                       [ResidualBlockWithStride_wave(in_ch=2 * N, out_ch=M, stride=2, wavelet='haar')]

        self.m_up1 = [ConvTransBlock(dim, dim, self.head_dim[3], self.window_size, dpr[i+begin], 'W' if not i%2 else 'SW') 
                      for i in range(config[3])] + \
                      [Upblock_SAL(2*N, 2*N, kernel_size=5, stride=2, groups=1, num_heads=12)]
        self.m_up2 = [ConvTransBlock(dim, dim, self.head_dim[4], self.window_size, dpr[i+begin], 'W' if not i%2 else 'SW') 
                      for i in range(config[4])] + \
                      [Upblock_SAL(2*N, 2*N, kernel_size=5, stride=2, groups=1, num_heads=6)] + \
                      [SFMA(in_dim=2 * N, middle_dim=64, adapt_factor=1)]
        self.m_up3 = [ConvTransBlock(dim, dim, self.head_dim[5], self.window_size, dpr[i+begin], 'W' if not i%2 else 'SW') 
                      for i in range(config[5])] + \
                     [ResidualBlockUpsample_wave(in_ch=2 * N, out_ch=3, upsample=2, wavelet='haar')]

        self.g_a = nn.Sequential(*[Downblock_SAL(4, 2*N, kernel_size=5, stride=2, groups=1, pool_stride=32, num_heads=3)] + self.m_down1 + self.m_down2 + self.m_down3)
        

        self.g_s = nn.Sequential(*[Upblock_SAL(M, 2*N, kernel_size=5, stride=2, groups=1, num_heads=24)] + self.m_up1 + self.m_up2 + self.m_up3)
        ...
        self.h_a = nn.Sequential(
            *[Downblock_SAL(320*4, 2*N, kernel_size=5, stride=2, groups=1, pool_stride=4, num_heads=24)] + \
            self.ha_down1
        )
        ...
        self.h_mean_s = nn.Sequential(
            *[Upblock_SAL(192, 2*N, kernel_size=5, stride=2, groups=1, num_heads=3)] + \
            self.hs_up1
        )
...
#通道熵模块
    ...
    def update(self, scale_table=None, force=False):        #该方法主要用于更新对象的状态，特别是与高斯条件分布相关的尺度表。它可以在需要更新尺度表时被调用，以确保对象使用最新的尺度表进行计算或处理。
    ...
    def forward(self, x1, x2):                     #前向传播
        # print(x1.shape)                          #  x1=(8,3,256,256)
        # print(x2.shape)                          #  x2=(8,1,256,256)
        x = torch.cat((x1, x2), dim=1)
        # print(x.shape)                           #  x=(8,4,256,256)
        y = self.g_a(x)
        # print(y.shape)                           #  y=(8,320,16,16)

        y_shape = y.shape[2:]
        y_output = self.dwt(y)
        #print(y_output.shape)                      #  y_output=(8, 1280, 8, 8)

        # low_frequency and high_frequency
        low_freq = y_output[:, :320, :, :]
        high_freq = y_output[:, 320:, :, :]

        # print(low_freq.shape)                       #  low_freq=(8, 320, 8, 8)
        # print(high_freq.shape)                      #  high_freq=(8, 960, 8, 8)

        y_input = torch.cat([low_freq, high_freq], dim=1)
        # print(y_input.shape)                        #  y_output=(8, 1280, 8, 8)

        z = self.h_a(y_input)
        #print(z.shape)                              #   z=(8, 192, 2, 2)
        ...
        # print(x_hat.shape)                        # x_hat=(8,3,256,256)
        ...
        # print(z_hat.shape)                  #   z_hat=(8, 192, 2, 2)
...
#     #这个方法通常用于模型的加载和恢复训练。例如，在训练过程中，模型的状态（包括参数和缓冲区）会被定期保存。当训练中断后，可以使用load_state_dict方法加载保存的状态，从而继续训练。它也可以用于模型的迁移学习，即将一个预训练模型的参数加载到新的模型中，以便在新任务上进行微调。
...
#
# #这个函数的主要用途是从一个保存的模型状态字典 (state_dict) 中恢复一个模型实例。这在模型保存和加载、迁移学习或者模型继续训练时非常有用。
#     @classmethod
#     def from_state_dict(cls, state_dict):
#         """Return a new model instance from `state_dict`."""
...
#压缩
    def compress(self, x1, x2):
        # print(x1.shape)    #torch.Size([1, 3, 512, 640])
        # print(x2.shape)     #torch.Size([1, 1, 512, 640])
        x = torch.cat((x1, x2), dim=1)
        # print(x.shape)      #torch.Size([1, 4, 512, 640])
        y = self.g_a(x)
        y_shape = y.shape[2:]
        ...
        # print(y_input.shape)      #torch.Size([1, 1280, 16, 20])
        z = self.h_a(y_input)
        # print(z.shape)

        z_strings = self.entropy_bottleneck.compress(z)                             # z_strings比特流
        ...
        for slice_index, y_slice in enumerate(y_real_slices):
            support_slices = (
                y_real_hat_slices if self.max_support_slices < 0 else y_real_hat_slices[:self.max_support_slices])
            mean_support = torch.cat([latent_means] + support_slices, dim=1)
            mean_support = self.atten_mean_real[slice_index](mean_support)
            mu = self.cc_mean_transforms_real[slice_index](mean_support)
            mu = mu[:, :, :, :]
        ...
        for slice_index, y_slice in enumerate(y_imag_slices):
            support_slices = (
                y_imag_hat_slices if self.max_support_slices < 0 else y_imag_hat_slices[:self.max_support_slices])
            mean_support = torch.cat([latent_means] + [y_real_hat] + support_slices, dim=1)
            mean_support = self.atten_mean_imag[slice_index](mean_support)
            mu = self.cc_mean_transforms_imag[slice_index](mean_support)
            mu = mu[:, :, :, :]
...
if __name__ == '__main__':
    ...
    # print(output[first_key].shape)


    #压缩通道调试程序     图像要是256的倍数
    x1 = torch.randn(1, 3, 512, 768)
    x2 = torch.randn(1, 1, 512, 768)
    tcm = TCM()
    tcm.update()
    output = tcm.compress(x1,x2)
