...
class Fusionloss(nn.Module):
    ...
    def forward(self, generate_img, image_vis, image_ir):  # label
        N, _, H, W = image_vis.size()
        out = {}
        num_pixels = N * H * W
        out["bpp_loss"] = sum(
            (torch.log(likelihoods).sum() / (-math.log(2) * num_pixels))
            for likelihoods in generate_img["likelihoods"].values()
        )


        image_y = image_vis[:, :1, :, :]
        generate = generate_img["x_hat"]
        generateimg = generate[:, :1, :, :]

        x_in_max = torch.max(image_y, image_ir)
        loss_in = F.l1_loss(x_in_max, generateimg)
        out["loss_in"] = loss_in

        y_grad = self.sobelconv(image_y)
        ir_grad = self.sobelconv(image_ir)
        generate_img_grad = self.sobelconv(generateimg)
        ...
        f_LL, f_LH, f_HL, f_HH = self.pool(generateimg)
        ...
        out["loss_fre_total"] = loss_fre_total

        out["loss"] = 12 * out["loss_in"] + 15 * out["loss_grad"] + 15 * out["loss_fre_total"] + out["bpp_loss"]
        return out
